...
with a constant-rate uplift. Escarpment landscape
with E and W boundaries open, N and S closed
...
data_output_square = os.path.join(data_output_ss, dirs["square"])
...
data_savedir = os.path.join(data_output_square, save_filepath)
...
def evolve_block_to_steady_state(mg, initial_condition, evolve_dict,
    ...
    # ss_thresh = forward_params['steady_state_threshold']
    # seed = ic_params['seed']
    # colour = ic_params['colour']
    ...
    for step in np.arange(n_steps):
        ...
        # Copy the elevations, every 10 timesteps
        if step % 10 == 0:
...
landscapes, ss_data = evolve_block_to_steady_state(grid_1, start_topo, evolvers,
